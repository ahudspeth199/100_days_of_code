...
Our snake should be able to head a piece of food, which is just going to be a blue circle.
...
Now, everything that's to do with a snake, it's a parent's and behavior, is all captured inside a class:
"""
...
## Step 1
"""
So everything snake related is inside the snake class. So what we want to be able to do is to create a new food.py file
(from me it will be food189.py) and this food.py is going to be its own class.
"""
class Food:

"""
And the food class is going to know how to render itself as a small circle on the screen.
...
And this piece of food that we're going to see onscreen is going to be a turtle object. 

Instead of creating it as an attribute in this class, so """self.food = turtle""" 
like this, what we want to be able to do is we actually want this """class Food""", to inherit from the turtle class.
...
"""
All right.
So we mentioned that there's only two steps we need to do. 
First is after the name of the class we add some parentheses and then put in the name of the class that we want to
inherit from. 
...
And then the next thing we need to do is we need to call the turtle's init method inside the food's init method.
And you can see that as soon as you've added this inheritance here, this init starts giving us a warning.
And when you click on it, you can see it says call to __init__ of superclass is missed.
...
So it actually knows what we need to do next. That means if we click on the light bulb,
we can actually automatically just add in the superclass' call.
This, of course, you could just type it out and it's good for practice,
but in case you're ever wondering if you get these warnings, you can always take a look at what PyCharm is recommending.
...
and it knows what shape is. This is a method that the turtle class has that I'm now going to modify in my food class.
...
It's normally 20 by 20 pixels, but I wanna turn it into 10 by 10 pixels.So I'm going to stretch the length by 0.5,
so I'm basically going to half it.
And then I'm going to stretch the width also by 0.5. So now it should be a 10 by 10 circle 
which I've created by defining the shape size. 

Now, remember all of these methods, shape, size, pen up, shape, 
comes from this turtle superclass and we're only able to use it because we're inheriting 
from the superclass so that our food class is now also sort of a turtle, but it's more like a souped-up turtle. 

Finally, it let's go ahead and define the color which I'm going to set as blue, but of course, 
feel free to set it as anything you like.

And I'm also going to set the speed of my turtle to fastest. 
...
Speaking of moving, we're going to need to use the goto to get it to go to a random X, Y location.
"""
from turtle import Turtle
import random
...
"""
Remember that our screen is 600 by 600 so that means our X-axis goes from -300 to +300
...
Back in our main.py right below where we initialized our snake, we're going to initialize our food.
...
"""
from food189 import Food
"""
Another thing you'll notice is that this turtle is now grayed out because we're
not using that class anywhere inside the main.py. So we can delete that which gets rid of all our warnings on this page
and we get the green checkmark. Our food.py looks good, our snake.py looks good,
...
into contact and then to tell the food to move itself to a new random location? 

Well, we're going to do that inside our main.py. 
...
snake's head to the food. At this point, you can check to see if it is less than a certain amount,
then it's pretty likely that the snake head is now colliding with the food.
So we know that the food is 10 by 10. So if we add a bit of a buffer, let's just say,
if this snake head is within 15 pixels of the food or even closer, so if the distance is less than 15,
...
and if I now go and touch this piece of food, then you can see nom nom nom being printed in the console.

Now let's go ahead and touch it again. You can see, as soon as I collide with that food, I get that print statement executing.
...
So be careful that you didn't right reset because that's one of the methods from the turtle class that we're inheriting from.
"""
from turtle import Turtle
import random
...
And then back inside our main.py, when the snake head collides with the food,
...
then you can see the food now appears at a new, random location. Like that. 

Now, depending on how accurate you want the snake to hit the food,
so it might need to hit it dead on, or if you're happy with it just gliding past and counting that as a collision,
then you could decrease the number here:
...
Now I've done a bit of testing and this number seems to be the best distance in order to get a valid collision,
...
Now I know that in the snake game, the snake segments increase once it hits a piece of food,
but we're only going to add that functionality at the very end of this project
when we're detecting collision with the snake tail. So don't worry about that for now.
